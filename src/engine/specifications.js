...
export const SPEC_SOURCE = "specification_csv";
...
export function modelMatchKey(brand, title) {
...
export function parseThrowRatio(raw) {
...
export function parseBrightness(raw) {
...
export function parseResolution(raw) {
  if (blank(raw)) {
...
export function parseInputLag(raw) {
  if (blank(raw)) return null;
...
export function parseNoise(raw) {
  if (blank(raw)) return null;
...
export function parseConnectivity(raw) {
  const t = String(raw || "").toLowerCase();
  return {
    hdmiCount: (() => {
...
export function parseSuitableFor(raw) {
  const t = String(raw || "").toLowerCase();
  return {
...
export function plusMinHints(plus, min) {
...
export function inferThrowClassFromRatio({ throwRatioMin, throwRatioMax, ust, shortThrow, title }) {
...
export function parseSpecRow(raw) {
...
export function matchSpecsToProducts(specRows, products) {
  const report = {
...
export function applySpecToProduct(product, spec, method) {
  const conflicts = [];
...
export function coverageSnapshot(products) {
  const projectors = products.filter((p) => p.productKind === "projector");
...
export function enrichProductList(products, specRows) {
